...
function getCriterias(req, res) {
  ...
  var query = {};
  query.where = {permission: permissionId};
  ...
  Criteria.find(query)
    ...
    .then(function (criterias) {
      ...
      res.ok({criterias: criterias, more: more});
    })
    .catch(function (err) {
      sails.log.error(err);
      res.send(500, {
...
function getMyCriterias(req, res) {
  ...
  var query = {};
  query.where = {criteriaOwner: ownerId};
  if (filter)
    query.where.or = [{'name': {'contains': filter}},
      {'notes': {'contains': filter}}];
...
function updateCriteria(req, res) {
  ...
  Criteria.update({id: id}, criteria)
