...
(function(angular) {
  ...
  function route($stateProvider, $httpProvider, $ionicConfigProvider) {
    ...
    .state('Main.Poll.PollList', {
      url: '/PollList',
      views: {
        Poll: {
          templateUrl: 'state/Poll/PollList/PollList.html',
          controller: 'PollListController as PollList'
        }
      }
    })
  ...
  function AppText() {
    return function(input) {
      ...
      if (input === 'title') {
        ...
        //  Field
        //====================================================
      } else if (input === 'like') { //common Field, Club, Ball
        return '좋아요';
        ...
        //  QuestionAnswer
        //====================================================
      } else if (input === 'courseCondition') {
        return '코스관리';
      } else if (input === 'process') {
        return '진행';
      } else if (input === 'caddie') {
        return '캐디';
      } else if (input === 'facility') {
        return '시설관리';
      } else if (input === 'functionality') {
        return '기능';
      } else if (input === 'feel') {
        return '감각';
...
(function(angular) {
  'use strict';
  angular.module('app')
    .factory('PollAnswers', PollAnswers);
  ...
  function PollAnswers(
    ...
    var pollUrl = SERVER_URL + '/pollAnswer' +
      '/:uri';
    ...
    var service = $resource(pollUrl, params, actions);
...
(function (){
  ...
  function PollListController(
    ...
    PollList.getColWidth = getColWidth;
    ...
    function onAfterEnter() {
      if (!U.hasPreviousStates(noLoadingStates)) {
        return initPromise
          .then(function(pollAnswersWrapper) {
            console.log("---------- pollAnswersWrapper ----------");
            console.log(pollAnswersWrapper);
            U.bindData(pollAnswersWrapper, PollListModel, 'pollAnswers');
          })
          .catch(function(err) {
            U.error(err);
          });
      } else {
        U.scrollTo(PollListModel);
      }
    }
    ...
    function pollAnswerFind() {
      ...
      return PollAnswers.find(queryWrapper).$promise
        .then(function(pollAnswersWrapper){
          return pollAnswersWrapper;
        });
    }
...
(function() {
  ...
  angular.module('app')
    .factory('PollListModel', PollListModel);

  PollListModel.$inject = [];

  function PollListModel() {
          ...
          .then(function(post) {
            ...
            $timeout(function() {
              if (PostUpdateModel.post.category === 'EVENT' ||
                PostUpdateModel.post.category === 'NOTICE'
              ) {
...
(function() {
  ...
  function QuestionAnswerListController(
    ...
    QuestionAnswerList.order = order;
    ...
    function order(name){
      ...
      $scope.reverse = ($scope.predicate === name) ? !$scope.reverse : false;
      QuestionAnswerListModel.questionanswers = 
        orderBy(QuestionAnswerListModel.questionanswers, $scope.predicate, $scope.reverse);
    }
    ...
    function averageArrays(questionAnswers){
      var questionsPerOwnerObj = _.groupBy(questionAnswers, 'createdBy');
      var averageResults =[];
      var key;
      var labels;
      if(QuestionAnswerListModel.selectedCategory === 'FIELD'){
        // labels = ['코스관리상태', '진행', '캐디', '시설관리', '기타'];
        labels = ['courseCondition', 'process', 'caddie', 'facility', 'etc'];
      } else if(QuestionAnswerListModel.selectedCategory === 'CLUB'){
        // labels = ['기능', '감각', '디자인', '이미지', '추천도'];
        labels = ['functionality', 'feel', 'design', 'image', 'recommended'];
      } else if(QuestionAnswerListModel.selectedCategory === 'BALL'){
        // labels = ['타구감', '스핀량', '디자인', '브랜드이미지', '가격대비만족도'];
        labels = ['hit', 'spin', 'design', 'brand', 'rightPrice'];
      }

      for(key in questionsPerOwnerObj) {
        averageResults.push( averageObj(questionsPerOwnerObj[key], labels) );
      }

      return averageResults;
    }

    function averageObj(questionAnswers, labels){
      var questions0 = _.filter(questionAnswers, function(q){return q.position === 0;});
      var questions1 = _.filter(questionAnswers, function(q){return q.position === 1;});
      var questions2 = _.filter(questionAnswers, function(q){return q.position === 2;});
      var questions3 = _.filter(questionAnswers, function(q){return q.position === 3;});
      var questions4 = _.filter(questionAnswers, function(q){return q.position === 4;});

      var question0Mean = _.meanBy(questions0, function(q){return q.score;});
      var question1Mean = _.meanBy(questions1, function(q){return q.score;});
      var question2Mean = _.meanBy(questions2, function(q){return q.score;});
      var question3Mean = _.meanBy(questions3, function(q){return q.score;});
      var question4Mean = _.meanBy(questions4, function(q){return q.score;});
      var result = {};
      result[labels[0]] = isNaN(question0Mean) ? 0 : _.round(question0Mean, 1);
      result[labels[1]] = isNaN(question1Mean) ? 0 : _.round(question1Mean, 1);
      result[labels[2]] = isNaN(question2Mean) ? 0 : _.round(question2Mean, 1);
      result[labels[3]] = isNaN(question3Mean) ? 0 : _.round(question3Mean, 1);
      result[labels[4]] = isNaN(question4Mean) ? 0 : _.round(question4Mean, 1);
      result.product = questions0[0].product;
      result.owner = questions0[0].owner;
      return result;
    }
    ...
    function questionanswerFind(extraQuery, extraOperation) {
      ...
      return QuestionAnswers.findQuestionAnswer(queryWrapper).$promise
        .then(function(questionanswersWrapper) {
          console.log("questionanswersWrapper --pre--:::\n", questionanswersWrapper.questionanswers);
          questionanswersWrapper.questionanswers = averageArrays(questionanswersWrapper.questionanswers);
          console.log("questionanswersWrapper --post--:::\n", questionanswersWrapper.questionanswers);
...
(function() {
  ...
  function QuestionAnswerListModel() {
    var Model = {
      ...
      columnFilterOption: {
        FIELD: [
          {name: 'courseCondition', show: true },
          {name: 'process', show: true },
          {name: 'caddie', show: true },
          {name: 'facility', show: true },
          {name: 'etc', show: true },
          {name: 'owner', show: true }
        ],
        CLUB: [
          {name: 'functionality', show: true },
          {name: 'feel', show: true },
          {name: 'design', show: true },
          {name: 'image', show: true },
          {name: 'recommended', show: true },
          {name: 'owner', show: true }
        ],
        BALL: [
          {name: 'hit', show: true },
